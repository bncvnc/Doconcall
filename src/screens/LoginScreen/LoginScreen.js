...
onFieldTextChange = (text,field) => {
  ...
  getMyValue = async () => {
    ...
    // let converted =btoa('Unclefixer:6159d7354e6c39b817ef38707f9e9562');
    ...
    try {
     ...
     let data=JSON.parse(value);
      ...
      if(data.isLoggedIn){
        ...
        if(data.isLoggedIn && data.isVarified)
        {
        ...
        }else if(!data.isVarified){
          this.props.checkIfThereIsdata(data);
          setTimeout(()=>{
            SplashScreen.hide();
            
          },500)
          appNavigation('doconcall.verifiNumber')
        }
  ...
  render() {
    ...
    shadowOpacity: .5,
            ...
            <View
             ...
             <Icon style={{fontSize:wp('8%'), color:appColor}} name="lock" />
            </View>
      ...
      >{this.state.inputs.password.warningText}</Text>
        ...
        <TouchableOpacity  onPress={()=>this.changeScreenTo('doconcall.ForgetPassword','Forget Password')} style={styles.forgot}>
          <View
            style={{
...
export default connect(mapStateToProps,mapsDespathToProps)(CusLogin);